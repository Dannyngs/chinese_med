<?php
require_once 'init.php';

?>


<?php require_once 'header.php' ?>
<style type="text/css">
    .title{ background: url('_/img/price-tit.jpg') repeat-x top;}
</style>
<section class="title box">

  <h2>治療及價目</h2>
  

</section>


<div class="content_box">


  <!-- Sidebar layout -->
  <div class="sidebar_layout">


    <!-- Sidebar -->
    <div class="left_sidebar sidebar clearfix">
      
      <section>
        <h3>治療價目表</h3>
          <ul class="price">
            <li>開藥 100元</li>
              <li>針灸 300元</li>
<li>濕疹微電治療 300⾄至1400元<br/>
    (視乎濕疹面積及治療時間長短)</li>
              <li>上門治療 1000至2000元</li>
              <li>經筋鬆解推拿治療 400元/50分鐘</li>
              <li>紅外線照燈治療 150元/25分鐘</li>
              <li>微電貼治療 100元</li>
              <li>耳穴治療 150元</li>
              <li>中醫纖體治療 700元</li>
<li>中醫面部肌膚改善治療 700元</li>
          </ul>
        
      </section>
      

      <section>
        <h3>藥物及產品價目表</h3>
        <ul class="price">
            <li>全新微電貼 每對100元</li>
          <li>皮膚藥膏 150丶250 或 350元<br/>
            (治療濕疹，絶對不含類固醇)</li>
            <li>浸浴藥粉 150⼂丶250 或 350元</li>
            <li>藏紅花 每瓶180元</li>
            <li>微電機租借之按金 1800元</li>
            <li>微電機租借之月費 450元</li>
          </ul>
       
      </section>
        
        <section class="about">
        <h3>黎凱明註册中醫<br/><br/>
內科·婦科·兒科·皮膚·運動創傷</h3>
        <p>香港銅鑼灣希慎道8號裕景商業中心2樓</p>
            <p><label>Tel : </label>(852) 2889 6080 </p>
            <p><label>Mobile : </label>(852) 9020 8382</p>
            <p><label>acebook : </label>黎凱明中醫診所</p>
             <h3><a href="#">敬請預約 >> </a></h3>
      </section>


    </div>


    <!-- Main column -->
    <div class="main_column two_thirds">

      <!-- Features -->
      <section>
       <ol class="pri-right">
         <li><a href="">經筋鬆解推拿療法</a></li>
         <li><a href="">經筋鬆解推拿療法</a></li>
         <li><a href="">經筋鬆解推拿療法</a></li>
         <li><a href="">經筋鬆解推拿療法</a></li>
         <li><a href="">經筋鬆解推拿療法</a></li>
         <li><a href="">經筋鬆解推拿療法</a></li>
         <li><a href="">經筋鬆解推拿療法</a></li>
         <li><a href="">經筋鬆解推拿療法</a></li>
       </ol>
        <ul class="features_icons features">
          <li>
            <i class="icon-fullscreen black big"></i>
            <h3>百年老字號</h3>
            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean orci dolor, tincidunt eu vulputate hendrerit, ullamcorper sed mi. Sed pellentesque mi eget tellus pellentesque convallis.</p>
          </li>
          <li>
            <i class="icon-envelope-alt black big"></i>
            <h3>專業服務團隊</h3>
            <p>Aenean orci dolor, tincidunt eu vulputate hendrerit, ullamcorper sed mi. Sed pellentesque mi eget tellus pellentesque convallis. Sed id dui ut orci accumsan lobortis vitae vitae justo. Nunc eu ligula quis diam lacinia dignissim.</p>
          </li>
          <li>
            <i class="icon-envelope-alt black big"></i>
            <h3>賓至如歸的服務</h3>
            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean orci dolor, tincidunt eu vulputate hendrerit, ullamcorper sed mi. Sed pellentesque mi eget tellus pellentesque convallis. Sed id dui ut orci accumsan lobortis vitae vitae justo. Nunc eu ligula quis diam lacinia dignissim.</p>
          </li>
        </ul>

      </section>


      <hr />
      

    </div>


  </div>


</div> 

<?php require_once 'footer.php' ?>
