  <nav  class="desktop">
              <ul>
<?php

  $rs = $db->query("SELECT* FROM menu where parent_id = 0");
    $menu = $rs->fetchAll();
                 
                   foreach ($menu as $item){
                     
                   echo "<li class='down-menu'>";
                       if('/'.$item['url']==$_SERVER[ 'REQUEST_URI'])
                           $selected='selected';
                       else $selected='';
                       if($item['type']=="page")
                        echo "<a class='".$selected."' href='".$item['url']."'>".$item[$current_lang.'_title']."</a>";
                        else if($item['type']=="sub"){
                             
                             echo "<a href='#'>".$item[$current_lang.'_title']."</a>";
                         $srs = $db->query("SELECT* FROM menu where parent_id =".$item['id']);
                       if($srs){
                       $submenu = $srs->fetchAll();
                     
                          echo "<ul>";
                              foreach ($submenu as $subitem){
                           if('/'.$subitem['url']==$_SERVER[ 'REQUEST_URI'])
                                        $selected='selected';
                                            else $selected='';
                                         echo "<li ><a class='".$selected."' href='".$subitem['url']."'>".$subitem[$current_lang.'_title']."</a>";

                                }
                            echo "</ul>";
                           
                       }  
                       }
                        else{
                             
                        echo "<a href='#'>".$item[$current_lang.'_title']."</a>";     
                          $rs = $db->query("SELECT* FROM ".$item['type']);   
                           if($rs){
                                   $subMenu = $rs->fetchAll();
                                    echo "<ul>";
                                    foreach ($subMenu as $subItem){
                                       
                                       if('/'.$item['url'].'?id='.$subItem['id']==$_SERVER[ 'REQUEST_URI'])
                                        $selected='selected';
                                            else $selected='';
                          
                                         echo "<li ><a class='".$selected."' href='".$item['url']."?id=".$subItem['id']."'>"
                                             
                                             .$subItem[$current_lang.'_'.$item['type']]."</a>";

                                    }
                                    echo "</ul>";
                           } 
                            
                            
                        }
                       
                      
                       
                       echo "</li>";
                    
                  }
 
?>
                  <li> <a href="/?lang=chi">中文</a>|</li>
                  <li style="margin-left:0"> <a href="/?lang=eng">Eng</a></li>
    </ul>
            
          </nav>
                   <!-- Tablet navigation -->
           